...
def main():
  ...
  try:
    with open(args.results_json_file, "r") as f:
      results_data = json.load(f)
  except json.JSONDecodeError as e:
    print(
        f"::error file={args.results_json_file}::Failed to parse results"
        f" JSON: {e}"
    )
    sys.exit(1)
  except OSError as e:  # Catch other potential file reading errors
    print(
        f"::error file={args.results_json_file}::Error reading results"
        f" JSON: {e}"
    )
    sys.exit(1)

  try:
    with open(args.baseline_yaml_file, "r") as f:
      baseline_data_full = yaml.safe_load(f)
  except yaml.YAMLError as e:
    print(
        f"::error file={args.baseline_yaml_file}::Failed to parse baseline"
        f" YAML: {e}"
    )
    sys.exit(1)
  except OSError as e:  # Catch other potential file reading errors
    print(
        f"::error file={args.baseline_yaml_file}::Error reading baseline"
        f" YAML: {e}"
    )
    sys.exit(1)
  ...
  if not baseline_data_full or not isinstance(baseline_data_full, dict):
    print(
        f"::warning file={args.baseline_yaml_file}::Baseline YAML is empty or"
        " not a dictionary. Skipping comparison."
    )
    sys.exit(0)

  if args.config_id not in baseline_data_full:
    print(
        f"::notice::No baseline found for config_id '{args.config_id}' in"
        f" {args.baseline_yaml_file}. Skipping comparison."
    )
    sys.exit(0)

  config_baselines = baseline_data_full[args.config_id]
  if not isinstance(config_baselines, dict):
    print(
        f"::warning file={args.baseline_yaml_file},title=Invalid Baseline"
        f" Structure::Baseline entry for '{args.config_id}' is not a"
        " dictionary. Skipping."
    )
    sys.exit(0)
  ...
  print(
      json.dumps(config_baselines, indent=2)
  )  # Using json.dumps for consistent pretty printing
  ...
  if not actual_metrics_container or not isinstance(
    ...
    summary_messages.append(
        ...
        f" or not a dictionary in '{args.results_json_file}'. Cannot perform"
        " comparison."
    )
  ...
  for metric_name, baseline_info in config_baselines.items():
    if not isinstance(baseline_info, dict) or not all(
      ...
      summary_messages.append(
          ...
          f" baseline for '{args.config_id}' is missing 'baseline_ms' or"
    ...
    try:
      ...
      summary_messages.append(
          ...
          f" baseline for '{args.config_id}' has non-numeric 'baseline_ms' or"
          " 'threshold'. Skipping."
      )
      continue
    ...
    if (
        ...
        or not isinstance(actual_metric_entry, dict)
    ...
    if actual_value_ms > allowed_upper_bound:
      ...
      summary_messages.append(
          "  ::error"
          f" file={args.results_json_file},title={error_title}::{error_details}"
      )
  ...
  # Print all collected summary messages
  print("\n".join(summary_messages))
