...
absl::StatusOr<std::unique_ptr<PjRtClient>> GetStreamExecutorGpuClient(
  ...
  gpu_run_options->set_distributed_client(options.distributed_runtime_client);
  ...
  for (const auto& device : device_topology_pair.first) {
    device_to_process_index[GlobalDeviceId(
        device->global_device_id().value())] = device->process_index();
  }
  gpu_run_options->set_device_to_process_index(
      std::move(device_to_process_index));
