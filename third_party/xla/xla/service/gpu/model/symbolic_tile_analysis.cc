...
struct OutputTilingInfo {
  ...
  // Same as above, but linearized, so for the example above it would look like:
...
SymbolicTileAnalysis::ComputeTiledHloInstructionsImpl(
  ...
  for (const std::unique_ptr<SymbolicTiledHloInstruction>& symbolic_tiled_hlo :
    ...
    if (compute_all_tile_offset_indexing_maps ||
      ...
      TF_ASSIGN_OR_RETURN(
          ...
          ComputeTileOffsetIndexing(
              *symbolic_tiled_hlo,
              output_tiling_info.linear_output_tile_offset_indexing, context_));
    }
    ...
    if (const auto* symbolic_fusion_tiling =
      ...
      TF_ASSIGN_OR_RETURN(
          ...
          symbolic_fusion_tiling->analysis_.ComputeTiledHloInstructionsImpl(
...
SymbolicTileAnalysis::ComputeTiledHloInstructions(
  ...
  return ComputeTiledHloInstructionsImpl(
      tile_parameters, constraints_are_known_satisfied,
