...
TEST(GpuExecutableTest, OuputInfoToAndFromProto) {
  ...
  EXPECT_THAT(output_info0.ToProto(), ::tsl::proto_testing::EqualsProto(R"pb(
                allocation_index: 42,
                passthrough: true)pb"));
  EXPECT_THAT(GpuExecutable::OutputInfo::FromProto(output_info0.ToProto()),
              ::tsl::testing::IsOkAndHolds(output_info0));
  ...
  EXPECT_THAT(output_info1.ToProto(), ::tsl::proto_testing::EqualsProto(R"pb(
                allocation_index: 43,
                alias_config {
                  parameter_number: 89,
  ...
  EXPECT_THAT(GpuExecutable::OutputInfo::FromProto(output_info1.ToProto()),
              ::tsl::testing::IsOkAndHolds(output_info1));
  ...
  EXPECT_THAT(output_info2.ToProto(), ::tsl::proto_testing::EqualsProto(R"pb(
                allocation_index: 44,
                passthrough: true,
                alias_config { kind: MAY_ALIAS }
              )pb"));
  EXPECT_THAT(GpuExecutable::OutputInfo::FromProto(output_info2.ToProto()),
              ::tsl::testing::IsOkAndHolds(output_info2));
}
